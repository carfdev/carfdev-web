...
type ToastEvent = ToastT | ExternalToast | ToastToDismiss;
...
class Observer {
  ...
  promise = <ToastData>(
    ...
    let id: string | number = data?.id ?? toastsCounter++;
      ...
      .then(async (response) => {
        ...
        if (isReactElementResponse) {
          ...
          const toastSettings: PromiseIExtendedResult = isExtendedResult
            ...
            : { message: promiseData, id };

          this.create({ type: "error", description, ...toastSettings });
          ...
          this.create({ type: "success", description, ...toastSettings });
        }
      })
      .catch(async (error) => {
        result = ["reject", error];
  ...
  custom = (jsx: () => React.ReactElement, data?: ExternalToast) => {
    const id = data?.id || toastsCounter++;
    this.create({ jsx: jsx(), id, ...data });
    return id;
  };
